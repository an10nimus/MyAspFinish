...
\end{figure}
При построении графиков учтено, что $f_2(0) < f_2(0)$ и $f_2(1) > f_1(1)$, следовательно, на интервале $(0,\, 1)$ существует $\hat{\gamma}'$ такая, что $f_1(\hat{\gamma}') = f_2(\hat{\gamma}')$.
...
\begin{subequations}\label{eq:ch5_case_2_ineqs_5}
	\begin{align}
		&\frac{4}{\varkappa - 1} < \frac{2}{\varkappa} < g_2 L,\\
		&\frac{4}{\varkappa - 1} < g_2 L < \frac{2}{\varkappa},\\
		&g_2 L < \frac{4}{\varkappa - 1} < \frac{2}{\varkappa}.
	\end{align}
\end{subequations}
...
\begin{itemize}
	\item Случай ``сильного'' сцепления, $4/(\varkappa - 1) < 2/\varkappa < g_2 L$
	\item Случай ``среднего'' сцепления, $4/(\varkappa - 1) < g_2 L < 2/\varkappa$
	\item Случай ``слабого'' сцепления, $g_2 L < 4/(\varkappa - 1) < 2/\varkappa$
\end{itemize}

\todo{Продолжение: описания случаев словами, схемами.}

\todo{(Далее)} Выясним, какой параметр отвечает за то, какое переходное напряжение будет меньше, $\tilde{U}$ или $\hat{U}'$.
